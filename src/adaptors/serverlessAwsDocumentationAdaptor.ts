...
export const convertToSwaggerPluginFormat = (
    ...
    Object.keys(swaggerDefinition.paths[path]).forEach(httpMethod => {
      ...
      if (swaggerDefinition.paths[path][httpMethod].requestBody) {
        convertedSwaggerDefinition = {
          ...
          requestModels: {
            ...parseRequestBody(
              swaggerDefinition.paths[path][httpMethod].requestBody
            )
          }
        };
      }
      Object.keys(swaggerDefinition.paths[path][httpMethod].responses).forEach(
        response => {
          const responseDefinition =
            swaggerDefinition.paths[path][httpMethod].responses[response];
          Object.keys(responseDefinition.content).forEach(
            (contentType: string) => {
              const responseDefinitionForContent =
                swaggerDefinition.paths[path][httpMethod].responses[response]
                  .content[contentType];
              const responseModelSchema = parseSchema(
                responseDefinitionForContent.schema
              );
...
export const parseSchema = (
  tsoaSchema: SwaggerModelSchemaProperty
): object | string => {
  let responseSchema: string | object = tsoaSchema;
  if (
    tsoaSchema.type &&
    tsoaSchema.type === "array" &&
    tsoaSchema.items &&
    tsoaSchema
  ) {
    ...
    responseSchema = tsoaSchema["$ref"].split("/")[
      tsoaSchema["$ref"].split("/").length - 1
    ];
  }
...
export const parseRequestBody = (tsoaRequestBody: object) => {
  ...
  Object.keys(tsoaRequestBody["content"]).forEach((contentType: string) => {
    requestModel[contentType] = parseSchema(
      tsoaRequestBody["content"][contentType].schema
    );
  });
  return requestModel;
};
